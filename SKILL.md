...
Preserve unrelated configuration. Restart the remote-control daemon after changing either setting. Do not claim that the policy is active until the daemon reports `status: running`.
...
Generate a pairing code only when the device is not already paired:
